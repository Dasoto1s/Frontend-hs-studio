...
const ProductosHombre = ({ toggleCarritoModal, showCarritoModal }) => {
  const [productos, setProductos] = useState([]);

  useEffect(() => {
    const obtenerProductosHombre = async () => {
      try {
        const response = await axios.get('http://localhost:8080/pagina-principal/productos/hombre');
        setProductos(response.data);
      } catch (error) {
        console.error('Error al obtener los productos de hombre:', error);
      }
    };
  ...
  return (
    <div className="modulo1">
      ...
      <div className="modulo1__sombra">
        <h2 className="modulo1__titulo">Hombre</h2>
